...
class TestNakedTwins():
    def __init(self):
        pass

    def test_naked_twins(self, board_before, expected_boards):
        
        print('\nInitial board\n')
        utils.display(board_before)

        results = solution.naked_twins(board_before)
        if results in expected_boards:
            print('\nPassed!')
        else:
            for i, board in enumerate(expected_boards):
                print(f'Expected board {i}\n')
                utils.display(board)
                print('\nMy result\n')
                utils.display(results)
                diffs = self.get_diff(board, results)
                print(f'\nDifferences board {i}:\n{diffs}\n')


    def get_diff(self, expected, results):
        assert len(expected) == len(results), 'Inputs don\'t match!'
        assert expected.keys() == results.keys(), 'Inputs don\'t match!'
...
if __name__ == '__main__':
    ...
    test = TestNakedTwins()
    test.test_naked_twins(before_naked_twins_1, possible_solutions_1)
